...
def create_folder(log_dir):
    ...
    if not os.path.exists(log_dir):
        ...
        log(config.log_file, 'WARNING: summary folder already exists...')
...
def train(config):
    ...
    log(config.log_file, 'num_train_files: '+str(len(train_dataset.filepaths)))
...
def test(config):
    ...
    labels, predictions = trainer.update_validation_accuracy(config, config.weights, test=True)
